...
namespace CommunityContracts.Core.NPC
{
    public class PamProfile
    {
        ...
        public static class PamContract
        {
            ...
            public static void PamStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("PamMakerInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("PamKegInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
