...
namespace CommunityContracts.Core.NPC
{
    public class ShaneProfile
    {
        ...
        public string CharacterName { get; set; } = "Shane";
        ...
        public static class ShaneContract
        {
            private static ShaneProfile profile = new ShaneProfile();
            public static async void OfferDetailedContract()
            {
                ...
                {
                    processingLine =
                        T("ShanePreparedMayo", new { count = profile.ProcessorsOperated, quality = profile.QualityName }) + "\n\n" +
                        T("ShanePackShipment");
                }
                ...
                Game1.currentLocation.createQuestionDialogue(
                    ...
                    async (farmer, answer) =>
                    {
                        if (answer == "Yes")
                        {
                            ...
                            StardewValley.NPC Shane = Game1.getCharacterFromName(profile.CharacterName);
                            if (Shane != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Shane);
            ...
            public static void ShaneIntroduction()
            {
                profile.NPCLevel = UpdateNPCLevel(profile.CharacterName);
                profile.ProcessorsOperated = CountProcessors("Mayonnaise Machine");
                ...
                string dialogText =
                    T("ShaneAskContractChickens", new { npc = profile.CharacterName, quantity = profile.Quantity, quality = profile.QualityName, item = profile.PreparedItemName });
            ...
            public static void ShaneStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("ShaneRancherInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("ShaneMayoMachineInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
