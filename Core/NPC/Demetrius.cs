...
namespace CommunityContracts.Core.NPC
{
    public class DemetriusProfile
    {
        ...
        public string CharacterName { get; set; } = "Demetrius";
        ...
        public async Task<List<Item>> GenerateProductShipmentWithDelay(IMonitor monitor)
        {
            ...
            if (ProcessorsOperated > 0)
            {
                ...
                {
                    var DriedMushrooms = ItemRegistry.Create("DriedMushrooms") as SObject;
                    if (DriedMushrooms != null)
                    {
                        var key = (DriedMushrooms.ParentSheetIndex, DriedMushrooms.Quality);
                        if (!itemMap.TryGetValue(key, out var stacked))
                        {
                            stacked = new SObject("DriedMushrooms", 1);
        ...
        public static class DemetriusContract
        {
            private static DemetriusProfile profile = new DemetriusProfile();
            public static async void OfferDetailedContract()
            {
                ...
                {
                    processingLine =
                        T("DemetriusPreparedItems", new { count = profile.ProcessorsOperated, quality = profile.QualityName, item = profile.PreparedItemName }) + "\n\n" +
                        T("DemetriusPackShipment");
                }
                ...
                Game1.currentLocation.createQuestionDialogue(
                    ...
                    async (farmer, answer) =>
                    {
                        if (answer == "Yes")
                        {
                            ...
                            StardewValley.NPC Demetrius = Game1.getCharacterFromName(profile.CharacterName);
                            if (Demetrius != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Demetrius);
            ...
            public static float EstimateContractValue()
            {
                ...
                profile.ProductOneMultiplier = 8.0f + (profile.NPCLevel * 1.00f);
            ...
            public static void DemetriusIntroduction()
            {
                profile.NPCLevel = UpdateNPCLevel(profile.CharacterName);
                profile.ProcessorsOperated = CountProcessors("Dehydrator");
                ...
                string dialogText =
                    T("DemetriusAskContractMushrooms", new { npc = profile.CharacterName, quantity = profile.Quantity, quality = profile.QualityName, item = profile.PreparedItemName });
            ...
            public static void DemetriusStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("DemetriusGrowerInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("DemetriusDehydratorInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
