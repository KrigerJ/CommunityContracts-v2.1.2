...
namespace CommunityContracts.Core.NPC
{
    public class SandyProfile
    {
        ...
        public string CharacterName { get; set; } = "Sandy";
        ...
        public static class SandyContract
        {
            private static SandyProfile profile = new SandyProfile();
            public static async void OfferDetailedContract()
            {
                ...
                {
                    processingLine =
                        T("SandyFermentedProducts", new { count = profile.ProcessorsOperated, quality = profile.QualityName, item = profile.PreparedItemName }) + "\n\n" +
                        T("SandyPackShipment");
                }
                ...
                Game1.currentLocation.createQuestionDialogue(
                    ...
                    async (farmer, answer) =>
                    {
                        if (answer == "Yes")
                        {
                            ...
                            StardewValley.NPC Sandy = Game1.getCharacterFromName(profile.CharacterName);
                            if (Sandy != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Sandy);
            ...
            public static void SandyIntroduction()
            {
                ...
                string dialogText =
                    T("SandyAskContractDesert", new { npc = profile.CharacterName, quantity = profile.Quantity, quality = profile.QualityName, item = profile.PreparedItemName });
            ...
            public static void SandyStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("SandySpecialistInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("SandyKegInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
