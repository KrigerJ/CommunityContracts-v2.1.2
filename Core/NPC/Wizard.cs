...
namespace CommunityContracts.Core.NPC
{
    public class WizardProfile
    {
        ...
        public string CharacterName { get; set; } = "Wizard";
        ...
        public static class WizardContract
        {
            private static WizardProfile profile = new WizardProfile();
            public static async void OfferDetailedContract()
            {
                ...
                {
                    processingLine =
                        T("WizardPreparedMusk", new { count = profile.ProcessorsOperated, quality = profile.QualityName }) + "\n\n" +
                        T("WizardPackShipment");
                }

                string dialogText =
                    T("WizardOfferContract", new { npc = profile.CharacterName, quantity = profile.Quantity, item = profile.PreparedItemName }) + "\n\n" +
                    processingLine + "\n\n" +
                ...
                Game1.currentLocation.createQuestionDialogue(
                    ...
                    async (farmer, answer) =>
                    {
                        if (answer == "Yes")
                        {
                            ...
                            StardewValley.NPC Wizard = Game1.getCharacterFromName(profile.CharacterName);
                            if (Wizard != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Wizard);
            ...
            public static float EstimateContractValue()
            {
                ...
                profile.ProductOneMultiplier = 1.5f + (profile.NPCLevel * 0.50f);
                profile.ProductTwoMultiplier = 2.5f + (profile.NPCLevel * 0.25f);
                profile.BaseProductMultiplier = 1.0f + (profile.NPCLevel * 0.20f);

                totalValue += (int)((profile.ItemPrice * profile.QualityMultiplier * profile.ProcessorsOperated) + (profile.ItemPrice * profile.Quantity));

                totalValue = GetTotalValue(totalValue);

                return totalValue;
            }
            public static void WizardIntroduction()
            {
                profile.NPCLevel = UpdateNPCLevel(profile.CharacterName);
                profile.ProcessorsOperated = CountProcessors("Oil Maker");
                ...
                string dialogText =
                    T("WizardAskContractObtained", new { npc = profile.CharacterName, quantity = profile.Quantity, item = profile.PreparedItemName });
            ...
            public static void WizardStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("WizardCrafterInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("WizardOilMakerInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
