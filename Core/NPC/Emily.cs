...
namespace CommunityContracts.Core.NPC
{
    public class EmilyProfile
    {
        ...
        public string CharacterName { get; set; } = "Emily";
        ...
        public static class EmilyContract
        {
            private static EmilyProfile profile = new EmilyProfile();
            public static async void OfferDetailedContract()
            {
                ...
                {
                    processingLine =
                        T("EmilyPreparedCloth", new { count = profile.ProcessorsOperated, quality = profile.QualityName }) + "\n\n" +
                        T("EmilyPackShipment");
                }
                ...
                Game1.currentLocation.createQuestionDialogue(
                    ...
                    async (farmer, answer) =>
                    {
                        if (answer == "Yes")
                        {
                            ...
                            StardewValley.NPC Emily = Game1.getCharacterFromName(profile.CharacterName);
                            if (Emily != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Emily);
            ...
            public static void EmilyIntroduction()
            {
                profile.NPCLevel = UpdateNPCLevel(profile.CharacterName);
                profile.ProcessorsOperated = CountProcessors("Loom");
                ...
                string dialogText =
                    T("EmilyAskContractSheep", new { npc = profile.CharacterName, quantity = profile.Quantity, quality = profile.QualityName, item = profile.PreparedItemName });
            ...
            public static void EmilyStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("EmilyFarmerInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("EmilyLoomInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
