...
namespace CommunityContracts.Core.NPC
{
    public class ElliottProfile
    {
        ...
        public string CharacterName { get; set; } = "Elliott";
        ...
        public async Task<List<Item>> GenerateProductShipmentWithDelay(IMonitor monitor)
        {
            ...
            if (ProcessorsOperated > 0)
            {
                ...
                {
                    var smoke = ItemRegistry.Create("SmokedFish") as SObject;
                    if (smoke != null)
                    {
                        var key = (smoke.ParentSheetIndex, smoke.Quality);
                        if (!itemMap.TryGetValue(key, out var stacked))
                        {
                            stacked = new SObject("SmokedFish", 1);
        ...
        public static class ElliottContract
        {
            private static ElliottProfile profile = new ElliottProfile();
            public static async void OfferDetailedContract()
            {
                ...
                {
                    processingLine =
                        T("ElliottSmokedItems", new { count = profile.ProcessorsOperated, quality = profile.QualityName, item = profile.PreparedItemName }) + "\n\n" +
                        T("ElliottPackShipment");
                }
                ...
                Game1.currentLocation.createQuestionDialogue(
...

                    {
                        if (answer == "Yes")
                        {
                            ...
                            StardewValley.NPC Elliott = Game1.getCharacterFromName(profile.CharacterName);
                            if (Elliott != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Elliott);
            ...
            public static void ElliottIntroduction()
            {
                profile.NPCLevel = UpdateNPCLevel(profile.CharacterName);
                profile.ProcessorsOperated = CountProcessors("Fish Smoker");
                ...
                profile.SObjectName = "SmokedFish";

                int[][] SeasonalCollect = new int[][]
                {
                    new int[] { 148, 267, 708 },
                    new int[] { 128, 130, 146, 149, 150, 155, 267, 701, 708 },
                    new int[] { 148, 150, 154, 155, 701, 705 },
                    new int[] { 130, 146, 150, 151, 154, 705, 708 }
                };
                ...
                string dialogText =
                    T("ElliottAskContractCatch", new { npc = profile.CharacterName, quantity = profile.Quantity, quality = profile.QualityName, item = profile.PreparedItemName });
            ...
            public static void ElliottStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("ElliottFisherInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("ElliottSmokerInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
