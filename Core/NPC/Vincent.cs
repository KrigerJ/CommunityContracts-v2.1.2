    ...
    public class VincentProfile
    {
        ...
        public async Task<List<Item>> GenerateProductShipmentWithDelay(IMonitor monitor)
        {
            ...
            if (Quantity > 0)
            {
                ...
                {
                    var processed = ItemRegistry.Create($"{SelectedItemID}") as SObject;
                    if (processed != null)
                    {
                        processed.Name = PreparedItemName;

                        var key = (processed.ParentSheetIndex, processed.Quality);
        ...
        public static class VincentContract
        {
                ...
                Game1.currentLocation.createQuestionDialogue(
                    ...
                    async (farmer, answer) =>
                    {
                        if (answer == "Yes")
                        {
                            ...
                            if (Vincent != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Vincent);
            ...
            public static float EstimateContractValue()
            {
                ...
                profile.Quantity = profile.ProcessorsOperated + profile.Quantity;

                totalValue += (int)(profile.ItemPrice * profile.ProductOneMultiplier * profile.QualityMultiplier * profile.Quantity);
            ...
            public static void VincentIntroduction()
            {
                ...
                profile.ProcessorsOperated = CountProcessors("Crab Pot");
            ...
            public static void VincentStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("VincentFishermanInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("VincentCrabPotInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
