...
namespace CommunityContracts.Core.NPC
{
    public class JodiProfile
    {
        ...
        public string CharacterName { get; set; } = "Jodi";
        ...
        public async Task<List<Item>> GenerateProductShipmentWithDelay(IMonitor monitor)
        {
            ...
            if (ProcessorsOperated > 0)
            {
                ...
                {
                    var ProcessedItem = ItemRegistry.Create("SmokedFish") as SObject;
        ...
        public static class JodiContract
        {
            private static JodiProfile profile = new JodiProfile();
            public static async void OfferDetailedContract()
            {
                ...
                {
                    processingLine =
                        T("JodiSmokedItems", new { count = profile.ProcessorsOperated, quality = profile.QualityName, item = profile.PreparedItemName }) + "\n\n" +
                        T("JodiPackShipment");
                }
                ...
                Game1.currentLocation.createQuestionDialogue(
                    ...
                    async (farmer, answer) =>
                    {
                        if (answer == "Yes")
                        {
                            ...
                            StardewValley.NPC Jodi = Game1.getCharacterFromName(profile.CharacterName);
                            if (Jodi != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Jodi);
            ...
            public static void JodiIntroduction()
            {
                ...
                {
                    new int[] { 137, 143, 706 },
	                new int[] { 138, 143, 144, 704, 706 },
	                new int[] { 137, 139, 140, 143, 699, 706 },
	                new int[] { 140, 141, 143, 144, 699, 707 }
 			    };
                ...
                string dialogText =
                    T("JodiAskContractFish", new { npc = profile.CharacterName, quantity = profile.Quantity, quality = profile.QualityName, item = profile.PreparedItemName });
            ...
            public static void JodiStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("JodiFisherInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("JodiSmokerInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
