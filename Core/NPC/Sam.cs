...
namespace CommunityContracts.Core.NPC
{
    public class SamProfile
    {
        ...
        public string CharacterName { get; set; } = "Sam";
        ...
        public static class SamContract
        {
            private static SamProfile profile = new SamProfile();
            public static async void OfferDetailedContract()
            {
                ...
                {
                    processingLine =
                        T("SamPreparedDuckMayo", new { count = profile.ProcessorsOperated, quality = profile.QualityName }) + "\n\n" +
                        T("SamPackShipment");
                }
                ...
                Game1.currentLocation.createQuestionDialogue(
                    ...
                    async (farmer, answer) =>
                    {
                        if (answer == "Yes")
                        {
                            ...
                            StardewValley.NPC Sam = Game1.getCharacterFromName(profile.CharacterName);
                            if (Sam != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Sam);
            ...
            public static void SamIntroduction()
            {
                ...
                string dialogText =
                    T("SamAskContractDucks", new { npc = profile.CharacterName, quantity = profile.Quantity, quality = profile.QualityName, item = profile.PreparedItemName });
            ...
            public static void SamStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("SamKeeperInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("SamMayoMachineInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
