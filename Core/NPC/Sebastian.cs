...
namespace CommunityContracts.Core.NPC
{
    public class SebastianProfile
    {
        ...
        public async Task<List<Item>> GenerateProductShipmentWithDelay(IMonitor monitor)
        {
            ...
            if (ProcessorsOperated > 0)
            {
                ...
                else
                {
                    for (int i = 0; i < ProcessorsOperated; i++)
                    {
                        var ProcessedItem = ItemRegistry.Create("447") as SObject;
                        if (ProcessedItem != null)
                        {
                            var key = (ProcessedItem.ParentSheetIndex, ProcessedItem.Quality);
                            if (!itemMap.TryGetValue(key, out var stacked))
                            {
                                stacked = new SObject("447", 1);
                                stacked.Quality = Quality;
        ...
        public static class SebastianContract
        {
            private static SebastianProfile profile = new SebastianProfile();
            public static async void OfferDetailedContract()
            {
                ...
                {
                    processingLine =
                        T("SebastianAgedRoe", new { count = profile.ProcessorsOperated, quality = profile.QualityName, item = profile.PreparedItemName }) + "\n\n" +
                        T("SebastianPackShipment");
                }
                ...
                Game1.currentLocation.createQuestionDialogue(
                    ...
                    async (farmer, answer) =>
                    {
                        if (answer == "Yes")
                        {
                            ...
                            StardewValley.NPC Sebastian = Game1.getCharacterFromName(profile.CharacterName);
                            if (Sebastian != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Sebastian);
            ...
            public static float EstimateContractValue()
            {
                ...
                profile.ProductOneMultiplier = 1.0f + (profile.NPCLevel * 0.50f);
                profile.ProductTwoMultiplier = 1.0f + (profile.NPCLevel * 0.50f);
            ...
            public static void SebastianIntroduction()
            {
                profile.NPCLevel = UpdateNPCLevel(profile.CharacterName);
                profile.ProcessorsOperated = CountProcessors("Preserves Jar");
                ...
                {
                    new int[] { 136, 143, 700, 702 },
                    new int[] { 136, 138, 143, 144, 698, 700, 702 },
                    new int[] { 136, 140, 143, 269, 700, 702 },
                    new int[] { 136, 140, 143, 144, 269, 698, 700, 702, 707 }
                };
                ...
                string dialogText =
                    T("SebastianAskContractCaught", new { npc = profile.CharacterName, quantity = profile.Quantity, quality = profile.QualityName, item = profile.PreparedItemName });
            ...
            public static void SebastianStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("SebastianFishermanInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("SebastianPreservesJarInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
