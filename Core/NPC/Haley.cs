...
namespace CommunityContracts.Core.NPC
{
    public class HaleyProfile
    {
        ...
        public string CharacterName { get; set; } = "Haley";
        ...
        public static class HaleyContract
        {
            private static HaleyProfile profile = new HaleyProfile();
            public static async void OfferDetailedContract()
            {
                ...
                {
                    processingLine =
                        T("HaleyPressedCheese", new { count = profile.ProcessorsOperated, quality = profile.QualityName }) + "\n\n" +
                        T("HaleyPackShipment");
                }

                string dialogText =
                    T("HaleyOfferContractCheese", new { npc = profile.CharacterName, quantity = profile.Quantity, quality = profile.QualityName, item = profile.PreparedItemName }) + "\n\n" +
                    processingLine + "\n\n" +
                ...
                Game1.currentLocation.createQuestionDialogue(
                    ...
                    async (farmer, answer) =>
                    {
                        if (answer == "Yes")
                        {
                            ...
                            StardewValley.NPC Haley = Game1.getCharacterFromName(profile.CharacterName);
                            if (Haley != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Haley);
            ...
            public static void HaleyIntroduction()
            {
                ...
                string dialogText =
                    T("HaleyAskContractCows", new { npc = profile.CharacterName, quantity = profile.Quantity, quality = profile.QualityName, item = profile.PreparedItemName });
            ...
            public static void HaleyStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("HaleyRancherInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("HaleyCheesePressInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
