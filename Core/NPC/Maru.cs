...
namespace CommunityContracts.Core.NPC
{
    public class MaruProfile
    {
        ...
        public string CharacterName { get; set; } = "Maru";
        ...
        public static class MaruContract
        {
            private static MaruProfile profile = new MaruProfile();
            public static async void OfferDetailedContract()
            {
                ...
                {
                    processingLine =
                        T("MaruPreparedDriedFruit", new { count = profile.ProcessorsOperated, quality = profile.QualityName, item = profile.PreparedItemName }) + "\n\n" +
                        T("MaruPackShipment");
                }
                ...
                Game1.currentLocation.createQuestionDialogue(
                    ...
                    async (farmer, answer) =>
                    {
                        if (answer == "Yes")
                        {
                            ...
                            StardewValley.NPC Maru = Game1.getCharacterFromName(profile.CharacterName);
                            if (Maru != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Maru);
            ...
            public static float EstimateContractValue()
            {
                ...
                profile.ProductTwoMultiplier = 1.00f + (profile.NPCLevel * 0.50f);
            ...
            public static void MaruIntroduction()
            {
                ...
                {
                    new int[] { 296, 634, 638 },
                    new int[] { 396, 398, 635, 636 },
                    new int[] { 398, 406, 410, 613, 637 },
                    new int[] { 414 }
                };
                ...
                string dialogText =
                    T("MaruAskContractCollected", new { npc = profile.CharacterName, quantity = profile.Quantity, quality = profile.QualityName, item = profile.PreparedItemName });
            ...
            public static void MaruStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("MaruSpecialistInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("MaruDehydratorInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
