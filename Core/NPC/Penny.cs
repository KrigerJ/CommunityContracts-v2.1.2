...
namespace CommunityContracts.Core.NPC
{
    public class PennyProfile
    {
        ...
        public string CharacterName { get; set; } = "Penny";
        ...
        public static class PennyContract
        {
            private static PennyProfile profile = new PennyProfile();
            public static async void OfferDetailedContract()
            {
                ...
                {
                    processingLine =
                        T("PennyCollectedHoney", new { count = profile.ProcessorsOperated, quality = profile.QualityName, item = profile.PreparedItemName }) + "\n\n" +
                        T("PennyPackShipment");
                }
                ...
                Game1.currentLocation.createQuestionDialogue(
                    ...
                    async (farmer, answer) =>
                    {
                        if (answer == "Yes")
                        {
                            ...
                            StardewValley.NPC Penny = Game1.getCharacterFromName(profile.CharacterName);
                            if (Penny != null)
                            {
                                Game1.player.changeFriendship((int)FriendshipAdd, Penny);
            ...
            public static float EstimateContractValue()
            {
                ...
                float HoneyMultiplier = GetHoneyMultiplier(profile.SelectedItemID);
                profile.QualityMultiplier = GetQualityMultiplier(profile.Quality);
                profile.ProductOneMultiplier = (1.0f + (profile.NPCLevel * 0.50f)) * HoneyMultiplier;
            ...
            public static void PennyIntroduction()
            {
                profile.NPCLevel = UpdateNPCLevel(profile.CharacterName);
                profile.ProcessorsOperated = CountProcessors("Bee House");
                ...
                string dialogText =
                    T("PennyAskContractGathered", new { npc = profile.CharacterName, quantity = profile.Quantity, quality = profile.QualityName, item = profile.PreparedItemName });
            ...
            public static void PennyStats()
            {
                int currentFriendship = Game1.player.friendshipData.TryGetValue(profile.CharacterName, out var data) ? data.Points : 0;

                string dialogText =
                    T("PennyBeekeeperInfo", new { npc = profile.CharacterName, level = profile.NPCLevel, quality = profile.QualityName }) + "\n\n" +
                    T("PennyBeeHouseInfo", new { npc = profile.CharacterName, count = profile.ProcessorsOperated }) + "\n\n" +
